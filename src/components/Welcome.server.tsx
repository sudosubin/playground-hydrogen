...
const DocsButton: FC<DocsButtonProps> = ({url, label}) => {
  return (
    <a
      href={url}
      target="_blank"
...
const StorefrontInfo: FC<StorefrontInfoProps> = ({
  shopName,
  totalProducts,
  totalCollections,
}) => {
...
interface TemplateLinksProps {
  firstProductPath: string;
  firstCollectionPath: string;
}

const TemplateLinks: FC<TemplateLinksProps> = ({
  firstProductPath,
  firstCollectionPath,
}) => {
  return (
...
const Welcome: FC = () => {
  const {data} = useShopQuery<WelcomeQueryResponse>({query: QUERY});
